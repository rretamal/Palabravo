...
public static partial class ReferralLink
{
    public static string? PuzzleFromCode(string? code)
    {
        ...
        if (int.TryParse(value, out var number) && number is >= 1 and <= 9999) return $"puzzle-{number:00}";
    ...
    [GeneratedRegex("^puzzle-(0[1-9]|[1-9][0-9]{1,3})$")] private static partial Regex PuzzleId();
}
