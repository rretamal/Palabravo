...
public sealed record MonetizationSettings
{
    ...
    private static MonetizationSettings Load()
    {
        ...
        if (settings.Enabled && !settings.TestAds && new[] { settings.AndroidBannerId, settings.AndroidInterstitialId,
            settings.AndroidRewardedId, settings.IosBannerId, settings.IosInterstitialId, settings.IosRewardedId }.Any(string.IsNullOrWhiteSpace))
